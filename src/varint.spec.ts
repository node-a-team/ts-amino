...
describe('Test varint', () => {
  it('uvarint should be encoded properly', () => {
    ...
    tests.forEach((test) => {
      assert.equal(new Uint8Array(uvarint.encode(test.int)).toString(), test.buf.toString())
  ...
  it('varint should be encoded properly', () => {
    ...
    for (let i = 0; i < 63; i += 1) {
      const int = bigInteger(1).shiftLeft(i).negate()
      const buf = varint.encode(int)
      assert.equal(varint.decode(buf).toString(), int.toString())
    }
    ...
    tests.forEach((test) => {
      assert.equal(new Uint8Array(varint.encode(test.int)).toString(), test.buf.toString())
